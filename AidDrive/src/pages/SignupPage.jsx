...
// to-do: delete files before reaching the backend
const deleteFiles = () => {
...
const SignupPage = () => {
  return (
    ...
    {/* make sure to validate  the captcha before submitting  */}
    ...
    <img onClick={deleteFiles} className='relative transition ease-in-out delay-150 top-[41%] hover:opacity-80 hover:-translate-y-1 hover:scale-[105%] cursor-pointer left-[92%] h-[35px] w-[35px]' src ={fileDelete}  />
    <span className='text-[14px] font-bold flex items-center justify-center h-auto sm:text-[20px] relative text-primary '>REGISTERATION DOCUMENTS</span>
    ...
    <label htmlFor="fileUploader" className='text-primary text-[14px] font-bold relative top-[43%] left-[32%]'>PNG and JPG formats only</label>
    <input type="file" id='fileInput' onChange={handleFileChange} required multiple accept='.pdf,.jpg,.png' id='fileUploader' className='text-transparent w-[400px] file:bg-primary file:h-[50px] file:w-[120px] file:border-none file:relative file:top-[50%] file:left-[50%] file:-translate-x-1/2  h-[200px] outline-dashed outline-primary rounded-[10px] relative left-[50%] top-[25%] -translate-x-1/2 -translate-y-1/2 file:cursor-pointer file:cursor-pointer file:rounded-[5px] file:font-bold file:hover:-translate-y-[30px] file:hover:scale-110 file:transition file:ease-in-out file:delay-150 file:hover:text-whitish file:text-whitish'/>
    <button type='submit' className='w-[250px] h-[80px] bg-accent rounded-[10px] font-bold text-primary text-[20px] translate-y-4 drop-shadow-md relative top-[50px] left-[50%] -translate-x-1/2  hover:scale-110 transition ease-in-out delay-150 '>SUBMIT</button>
    </form>
    </div>
    </div>
